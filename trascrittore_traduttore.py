...
def traduci_parole_con_gemini(testo: str, lingua_partenza: str, lingua_destinazione: str, parola: str) -> str:
    client = genai.Client(api_key=os.getenv('GEMINI_API_KEY'))
    # Rendiamo le istruzioni di sistema dinamiche basandoci sulle lingue passate
    word_prompt = (
        f"Sei un traduttore bilingue professionista ed esperto in {lingua_partenza} e {lingua_destinazione}. "
        f"Trova online tutti i siti web con parole in {lingua_partenza} e {lingua_destinazione} e scegli una parola casuale in {lingua_partenza}"
        ...
        f"Lascia esattamente 3 secondi di tempo per la risposta dell'utente"
    )
    ...
    system_prompt = (
        ...
        f"Lascia esattamente 3 secondi di tempo per la risposta dell'utente"
        f"Il testo inserito è in {lingua_partenza} (che potrebbe contenere espressioni gergali, dialettali o trascrizioni fonetiche). "
        f"Ignora qulsiasi rumore di sottofondo: analizza soltanto il suono più forte e più chiaro"
    )
